...
const Navbar = () => {
    ...
    const navLinks = [
        ...
        { name: 'Maintained Pages', path: '/maintained' },
                        ...
                        <div className="ml-10 flex items-baseline space-x-8">
                            {navLinks.map((link) => (
                                <Link
                                    key={link.name}
                                    to={link.path}
                                    className={`px-3 py-2 rounded-md text-sm font-medium tracking-wide transition-colors duration-200 ${location.pathname === link.path ? 'text-ananta-gold' : 'text-gray-200 hover:text-ananta-gold'}`}
                                >
                                    {link.name}
                                </Link>
                            ))}
            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        className="md:hidden bg-ananta-bg/95 backdrop-blur-xl border-b border-white/10 overflow-hidden"
                    >
